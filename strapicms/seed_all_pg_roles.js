...
client.connect().then(async () => {
  console.log('Connecting to PostgreSQL database yahaya_scool to grant ALL roles permissions...');
  
  const rolesRes = await client.query('SELECT id, name, type FROM up_roles');
  console.log('Granting permissions to roles:', rolesRes.rows.map(r => `${r.name} (${r.id})`).join(', '));
  ...
  console.log(`✅ ALL ROLES PERMISSION SEEDING COMPLETE! Granted ${linkedCount} role-permission links across ${rolesRes.rows.length} roles.`);
  client.end();
}).catch(err => {
