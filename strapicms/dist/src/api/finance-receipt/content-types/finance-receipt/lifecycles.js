...
function extractPrimitiveIdOrDoc(val) {
    ...
    if (typeof val === 'object') {
        if (val.documentId && typeof val.documentId === 'string')
            return val.documentId;
        if (val.id && (typeof val.id === 'number' || typeof val.id === 'string'))
            return val.id;
        if (Array.isArray(val.set) && val.set.length > 0)
            return extractPrimitiveIdOrDoc(val.set[0]);
        if (Array.isArray(val.connect) && val.connect.length > 0)
            return extractPrimitiveIdOrDoc(val.connect[0]);
        if (Array.isArray(val) && val.length > 0)
            return extractPrimitiveIdOrDoc(val[0]);
    }
    return null;
}
exports.default = {
    async afterCreate(event) {
        ...
        if (!result)
            return;
        const rawData = ((_a = event.params) === null || _a === void 0 ? void 0 : _a.data) || {};
        ...
        try {
            ...
            if (result.id && (invoice || student)) {
                try {
                    const receiptUpdate = {};
                    if (invoice)
                        receiptUpdate.invoice = invoice.id;
                    if (student)
                        receiptUpdate.student = student.id;
                    ...
                    strapi.log.warn('[Receipt Lifecycle] Failed linking receipt relations:', (err === null || err === void 0 ? void 0 : err.message) || err);
                }
            }
            ...
            if (invoice) {
                try {
                    ...
                    strapi.log.info(`[Receipt Lifecycle] Invoice ${invoice.invoiceNumber} (id=${invoiceId}) updated: paid=${newPaidAmount}, remaining=${newRemainingBalance}, status=${newStatus}`);
                }
                catch (err) {
                    strapi.log.error('[Receipt Lifecycle] Failed to update invoice:', (err === null || err === void 0 ? void 0 : err.message) || err);
                }
            }
            else {
            ...
            if (studentId && netWalletChange !== 0) {
                try {
                    ...
                    strapi.log.info(`[Receipt Lifecycle] Student ${student.schoolId} advance wallet: ${currentAdvance} → ${newAdvance} (change: ${netWalletChange > 0 ? '+' : ''}${netWalletChange})`);
                }
                catch (err) {
                    strapi.log.error('[Receipt Lifecycle] Failed to update student advance wallet:', (err === null || err === void 0 ? void 0 : err.message) || err);
                }
            }
                ...
                try {
                    ...
                    const ledgerType = overpaymentInPayCurrency > 0 && amountCoveringInvoice === 0
                        ...
                        : overpaymentInPayCurrency > 0
                            ? 'credit_with_advance'
                            : 'credit';
                    const description = overpaymentInPayCurrency > 0
                    ...
                    strapi.log.error('[Receipt Lifecycle] Failed to create ledger entry:', (err === null || err === void 0 ? void 0 : err.message) || err);
                }
            }
            ...
            try {
                ...
                strapi.log.warn('[Receipt Lifecycle] Journal entry failed:', (err === null || err === void 0 ? void 0 : err.message) || err);
            }
        }
        catch (err) {
            strapi.log.error('[Receipt Lifecycle] Critical afterCreate error:', (err === null || err === void 0 ? void 0 : err.message) || err);
        }
    },
};
