...
exports.default = strapi_1.factories.createCoreController('api::finance-journal-entry.finance-journal-entry', ({ strapi }) => ({
    async generateStatement(ctx) {
        ...
        try {
            ...
            if (manualEntries.length > 0) {
                ...
                lines.forEach((line) => {
                    var _a;
                    let codeMatch = (_a = line.account) === null || _a === void 0 ? void 0 : _a.match(/\((\d{4})\)/);
            ...
            const reportHash = crypto_1.default.createHash('sha256').update(JSON.stringify(balances) + Date.now()).digest('hex');
            return ctx.send({
                success: true,
                reportHash,
                totalDebits,
